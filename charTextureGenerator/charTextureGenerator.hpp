...
{
  Texture2D* toNewTexture(char myCharacter,int* imageDimensions)
  {
    std::cout<<"-"<<(int)myCharacter<<"-"<<std::endl;
    std::string folder = "C:/Users/matth/Documents/coding/OpenGL_resources/opengl_include/opengl_classes/charTextureGenerator/bitmaps/";
    std::string filename;
    ...
    Texture2D* charTexture = resourceManager::getCustomTexture(folder+filename,GL_LINEAR,GL_LINEAR,GL_LINEAR_MIPMAP_LINEAR,GL_LINEAR,GL_RED,GL_RGBA,imageDimensions);
    return charTexture;
  }
};
